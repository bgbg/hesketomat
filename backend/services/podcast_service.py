...
def create_podcast(db: Session, podcast: PodcastCreate) -> Podcast:
    logger.info(f"Creating podcast: {podcast.title} ({podcast.rss_url})")
    try:
        db_podcast = Podcast(
            title=podcast.title,
        ...
        logger.info(f"Successfully created podcast with ID: {db_podcast.id}")
        return db_podcast
...
def get_podcast_with_episode_count(db: Session) -> List[Dict]:
    logger.info("Getting podcasts with episode counts")
    try:
        results = (
            db.query(Podcast, func.count(Episode.id).label("episode_count"))
            .outerjoin(Episode)
            .group_by(Podcast.id)
            .all()
        )
        ...
        logger.error("Error getting podcasts with counts", exc_info=True)
        raise
...
def validate_rss_feed(
    ...
    try:
        ...
        if feed.bozo:
            logger.warning(f"Invalid RSS feed format: {rss_url}")
        ...
        if not hasattr(feed.feed, "title"):
            logger.warning(f"No title found in feed: {rss_url}")
            return False, "No title found in feed", None, None, None
        if not hasattr(feed.feed, "description"):
            logger.warning(f"No description found in feed: {rss_url}")
        ...
        logger.info(f"Successfully validated RSS feed: {rss_url} - {feed.feed.title}")
        return True, feed.feed.title, homepage_url, image_url, description
    except Exception as e:
        logger.error(f"Error validating RSS feed: {rss_url}", exc_info=True)
        raise
...
def update_podcast_episodes(db: Session, podcast: Podcast) -> List[Episode]:
    ...
    for entry in feed.entries:
        ...
        existing = (
            db.query(Episode)
            .filter(Episode.url == entry.link, Episode.podcast_id == podcast.id)
            .first()
        )
...
def delete_all_episodes(db: Session, podcast_ids: List[int]) -> None:
    db.query(Episode).filter(Episode.podcast_id.in_(podcast_ids)).delete(
...
def search_episodes(
    ...
    episodes = db.query(Episode).filter(Episode.podcast_id.in_(podcast_ids)).all()
            ...
            scored_episodes.append(
                {
                    ...
                    "matches": {
                        ...
                        "description": [
                            m.span() for m in pattern.finditer(episode.description)
                        ],
                    },
                }
            )
    ...
    return [
        {"episode": item["episode"], "matches": item["matches"]}
        for item in paginated_episodes
    ]
